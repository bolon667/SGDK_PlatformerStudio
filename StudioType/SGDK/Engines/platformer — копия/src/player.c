...
void playerInit(u16 positionNum) {
	...
	//playerBody.aabb = newAABB(4, 20, 4, 30);
	playerBody.aabb = newAABB(19, 30, 4, 30);

	//This collider is thinner because if the width is >=16 it could interfere with the lateral walls
	playerBody.climbingStairAABB = playerBody.aabb;
...
void playerInputChanged() {
	...
	if (joy == JOY_1) {
		...
		if (changed & (BUTTON_A | BUTTON_B | BUTTON_C)) {
			if(state & BUTTON_C){
				playerBody.attack = 1;
				addNewBullet((Bullet){TRUE, playerBody.globalPosition, {FIX16(playerBody.globalPosition.x), FIX16(playerBody.globalPosition.y)}, {FIX16(0.5),0}, {32,32}, {0,0,32,32}, FALSE, 1, NULL, &spr_coin});
			}
...
void updateAnimations() {
	...
	if(playerBody.attack){
		if(curPlayerAnim != 0){
			curPlayerAnim = 0;
			SPR_setAnim(playerBody.sprite, curPlayerAnim);
		}
		//playerBody.attack == 0;
		if((playerBody.sprite->frameInd == playerBody.sprite->animation->numFrame-1) ) {
		...
		if (playerBody.velocity.fixY == 0 && !playerBody.climbingStair) { 
			if (playerBody.velocity.x != 0  && playerBody.onGround) { //&& runningAnim == FALSE
				curPlayerAnim = 11; //run anim
				SPR_setAnim(playerBody.sprite, curPlayerAnim); 
			}else if (playerBody.velocity.x == 0 && playerBody.onGround) {
				curPlayerAnim = 7; //idle anim
				SPR_setAnim(playerBody.sprite, curPlayerAnim);
			}
		} else {
			if(playerBody.velocity.fixY < 0) { //if player jumping
				curPlayerAnim = 7; //jump anim
				SPR_setAnim(playerBody.sprite, curPlayerAnim);
			} else { // if player falling
				curPlayerAnim = 7; //falling anim
				SPR_setAnim(playerBody.sprite, curPlayerAnim);
	...
	//KLog_F1("123: ", playerBody.velocity.fixY);
	//Climb animation
	// if (playerBody.climbingStair) {
	// 	SPR_setAnim(playerBody.sprite, 8); //jump anim playerholder
	// }
}
