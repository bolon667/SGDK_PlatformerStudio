...
void playerInit(u16 positionNum) {
	...
	playerBody.aabb = newAABB(4, 46, 4, 46);
		...
		if (changed & (BUTTON_A | BUTTON_B | BUTTON_C)) {
			if(state & BUTTON_C){
				if(curPlayerAnim != 0){
					...
					addNewBullet((Bullet){TRUE, pos, {FIX16(pos.x), FIX16(pos.y)}, spd, {32,32}, {0,0,64,64}, FALSE, 1, NULL, &spr_cannonBall, 200});
				}
...
void updatePlayer() {
	...
	if (currentCoyoteTime > 0 && currentJumpBufferTime > 0) {
		...
		SND_startPlay_4PCM(sfx_jump, sizeof(sfx_jump), SOUND_PCM_CH1, FALSE);
