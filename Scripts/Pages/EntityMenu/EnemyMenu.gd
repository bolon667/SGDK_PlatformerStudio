...
func load_field_properties(entity_name: String, field_name: String):
	...
	match field_data["identifier"]:
		...
		"Trigger rect":
			...
			key = "defaultValue"
			val = field_data[key]
			field_property_node = field_property_triggerRect_t.instance()
...
func _on_AddNewEntityField_button_down():
	...
	singleton.add_field_to_entity_def(entityCollection, "entityInstances", singleton.cur_entity_type, final_field_name)
	load_entity_fields()
