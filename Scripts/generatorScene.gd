...
onready var ContainerRight: Control = get_tree().get_nodes_in_group("rightContainer")[0]
...
func _ready():
	...
	var entityInst_info = singleton.get_entityInst_by_instInd_levelInd(entityInst_id, level_ind, "Generator")
...
func _process(delta):
	if(singleton.cur_editor_mode == singleton.EditorMode.GENERATOR && !singleton.level_move_mode):
		move_entity()
		update()
...
func show_fields_of_entity():
	...
	var entity_definition = singleton.get_entityInst_by_instInd_levelInd(entityInst_id, level_ind, "Generator")
...
func move_entity():
	...
	if(Input.is_action_just_pressed("mouse1") && highlight):
		...
		singleton.cur_entity_inst_ind = singleton.get_entityInst_ind_by_id(entityInst_id, "Generator")
		remove_fields_of_entity()
		show_fields_of_entity()
		is_moving = true
		
		print("Found generator instInd: ", singleton.cur_entity_inst_ind)
		var entityInst_info = singleton.get_entityInst_by_instInd_levelInd(entityInst_id, level_ind, "Generator")
	if(is_moving):
...
func _on_Area2D_mouse_entered():
	if(singleton.cur_editor_mode == singleton.EditorMode.GENERATOR):
		singleton.cur_entity_inst_ind = entityInst_id
		add_to_group("generator_hovered")
		can_move = true

func _on_Area2D_mouse_exited():
	if(!is_moving):
		if(singleton.cur_editor_mode == singleton.EditorMode.GENERATOR):
			remove_from_group("generator_hovered")
			can_move = false
